...
Pass translated labels to the `translations` prop. Missing entries fall back to the built-in English labels.

```tsx
import { Comments, type Translations } from "@fuma-comment/react";

const translations = {
	"No comments(comment list)": "Aucun commentaire",
	"Sign In(auth button)": "Se connecter",
	"Today(timestamp)": "Aujourd'hui",
} satisfies Partial<Translations>;

export function CommentsWithAuth() {
	return <Comments page="default" auth={auth} translations={translations} />;
}
```

Use `satisfies Translations` instead when a locale should provide every label.
